...
const JoinUs: React.FC<JoinUsProps> = ({ show, onClose }) => {
  ...
  return (
    <div
      ref={modalRef}
      className="absolute top-[100px] left-00 right-00 h-[375px] bg-white z-[1000] shadow-[0_4px_8px_0_rgba(0,0,0,0.2)]"
    >
        ...
        <div className="flex justify-center items-center gap-2">
          {joinUsOptions.map((option, index) => (
            ...
            >
              ...
              />
              <div className="absolute inset-00 bg-gray-900 bg-opacity-30 opacity-0 group-hover:opacity-100 transition duration-300 ease-in-out" />
              <button
                className="absolute bottom-7 left-1/2 transform -translate-x-1/2 
                px-4 py-2 bg-gray-900 text-white rounded-full h-11 text-14 font-600 group-hover:bg-gray-500 transition-colors duration-300 ease-in-out z-10 shadow-lg"
              >
                {option.label}
              </button>
            </div>
